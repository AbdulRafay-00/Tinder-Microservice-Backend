...
export const options = {
    scenarios: {
        discovery_load_test: {
            ...
            stages: [
                ...
                { duration: '1m', target: 50 },    // sustained load, matches your earlier login test scale
                { duration: '1m', target: 100 },   // push further, matches your earlier 100-VU login test
    ...
    thresholds: {
        ...
        'http_req_failed': ['rate<0.05'], // fail the run only if something is badly broken (>5% errors)
    },
};
...
export default function discoveryLoadTest() {
    if (!token) {
        login();
    }
